...
def list_incidents(db: Session = Depends(get_db)) -> list[IncidentSchema]:
    return db.query(Incident).order_by(Incident.started_at.desc()).all()


@router.get("/{incident_id}", response_model=IncidentSchema)
def get_incident(incident_id: str, db: Session = Depends(get_db)) -> IncidentSchema:
...
def create_incident(payload: IncidentCreate, db: Session = Depends(get_db)) -> IncidentSchema:
